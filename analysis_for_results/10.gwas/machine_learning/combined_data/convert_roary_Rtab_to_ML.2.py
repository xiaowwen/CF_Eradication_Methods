...
add_pheno = True
input_genes = pd.read_csv(filepath_or_buffer='gene_presence_absence.Rtab', sep='\t', header=0, index_col=0)
genes       = input_genes.transpose()
phenotypes  = pd.read_csv(filepath_or_buffer='new_combined_resistances.pheno', sep='\t', header=0, index_col=0)
...
if add_pheno:
    ...
    merged_df.to_csv(path_or_buf="ml_combined_input.csv", index=False, header=False, sep='\t')
    merged_df.to_csv(path_or_buf="ml_combined_input.txt", index=True, header=True, sep='\t')

else:

    genes.to_csv(path_or_buf="ml_combined_input.csv", index=False, header=False, sep='\t')
    genes.to_csv(path_or_buf="ml_combined_input.txt", index=True, header=True, sep='\t')
