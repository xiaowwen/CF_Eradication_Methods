...
snp_pos     = snp_details #[ snp_detail.split('_')[2] for snp_detail in snp_details ]
chromosome  = 'NC_002516.2' #[ snp_detail.split('_')[0] + '_' + snp_detail.split('_')[1] for snp_detail in snp_details ]
...
for i in range(len(snp_pos)):
    ...
    ouput_list.append([chromosome, '.', snp_pos[i], pvalue[i], pvalue[i], 0])
